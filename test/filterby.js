...
function operandTest(op) {
  test("/resource?$filter=Foo " + op + " 2", "OData", function(result) {
...
test("/resource?$filter=Foo eq 'bar'", "OData", function(result) {
  it("A filter should be present", function() {
     assert.notEqual(result.options.$filter, null)
  })
...
test("/resource?$filter=Price gt 5 and Price lt 10", "OData", function(result) {
  ...
  it("Left hand side should be Price gt 5", function() {
     var lhs = result.options.$filter[1] 
  ...
  it("Right hand side should be less than 10", function() {
     var rhs = result.options.$filter[2] 
...
test("/resource?$filter=Price eq 5 or Price eq 10", "OData", function(result) {
...
test("/resource?$filter=not Published", "OData", function(result) {
...
test("/resource?$filter=not (Price gt 5)", "OData", function(result) {
...
test("/resource?$filter=Price add 5 gt 10", "OData", function(result) {
...
test("/resource?$filter=Price div Price mul 5 gt 10", "OData", function(result) {
  it("A filter should be present", function() {
     assert.notEqual(result.options.$filter, null)
  })
  it("Filter should be an instance of 'gt'", function() {
  ...
  var lexpr = result.options.$filter[1] 
...
function methodTestWithThreeArgs(methodName, expectFailure) {
  test("/resource?$filter=" + methodName + "('alfred', Product, 2) eq 'cake'", "OData", function(result, err) {
    if(expectFailure)
...
function methodTestWithTwoArgs(methodName, expectFailure) {
  test("/resource?$filter=" + methodName + "('alfred', Product) eq 'cake'", "OData", function(result, err) {
    if(expectFailure)
      it("Should fail because it's invalid", function() {
        assert.notEqual(err, null)
      })
...
function methodTestWithOneArg(methodName, expectFailure) {
  test("/resource?$filter=" + methodName + "('alfred') eq 'cake'", "OData", function(result, err) {
    if(expectFailure)
      it("Should fail because it's invalid", function() {
        assert.notEqual(err, null)
      })
